...
# Розміщення ПЕРЕВЕРНУТЕ проти версії 1309: імена, що НЕ влазять у 8.3
# (довгі, з пробілами, з малими літерами), ідуть ПЕРШИМИ записами кореня
# разом зі своїми LFN, а вже після них — робочі файли 8.3. Мітка тому йде
# останньою, щоб не займати перший запис.
#
...
IMG="$IMGDIR/720k-lba63-persha_$(date +%Y%m%d-%H%M%S).img"
...
# 1) спершу довгі імена — затичка йде найпершим записом кореня
...
# 3) мітка тому — останньою, щоб не забирати перший запис
label_at = i
root[i*32:(i+1)*32] = dirent(LABEL, 0x08, 0, 0, now)
i += 1
used_end = i
...
print()
print("затичка тепер ПЕРША, тому вінда доходить до неї, не спинившись на нулях.")
PYEOF

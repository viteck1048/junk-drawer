...
#
# ЧИСТИЙ 8.3, ЖОДНОГО ЗАПИСУ LFN У ТОМІ.
# ПЛЮС: файли за маскою 111* перейменовуються перед записом на випадкові
# імена завдовжки 6, 7 і 8 символів (розширення зберігається). Короткі
# чотирисимвольні імена читаються стійкою без помилок, тож перевіряємо,
# чи справа в довжині. У f0/ нічого не чіпається — перейменування живе
# лише всередині образу; таблиця відповідності друкується у звіті.
# Кладе файли з ~/agie-tmp/f0/, але тільки ті, чиї імена влазять у 8.3.
# Усе, що не влазить (довгі імена, пробіли, малі літери) — ПРОПУСКАЄТЬСЯ
# зі списком у звіті, на флешку не потрапляє. Затички проти вінди немає.
#
# Навіщо: вміст файлів на стійці ніколи не спотворювався, сипались лише
# імена в меню. Отже псується розбір каталогу, а не тракт даних. Записи
# LFN (attr 0x0F, UCS-2, набивка 0xFF) — єдине в корені, що не є звичайним
# 8.3. Ця версія прибирає їх повністю, щоб перевірити саме це.
#
# Порядок кореня: запис 0 — мітка тому, далі файли 8.3, далі нулі.
#
...
IMG="$IMGDIR/720k-lba63-rnd678_$(date +%Y%m%d-%H%M%S).img"
...
import os, struct, sys, time, random, fnmatch
...
RENAME_GLOB = '111*'                 # кого перейменовувати
RENAME_LENS = (6, 7, 8)              # довжини основи, по колу
...
for n in sorted(os.listdir(src)):
    ...
    rec = [n, p, st.st_size, st.st_mtime, ro, n]   # останнє поле — вихідне імʼя
    (short_files if is83(n) else long_files).append(rec)

# перейменування за маскою: випадкові основи 6/7/8 символів
busy = set(r[0].upper() for r in short_files)
k = 0
for r in short_files:
    if not fnmatch.fnmatch(r[5], RENAME_GLOB):
        continue
    ext = r[5].partition('.')[2]
    ln  = RENAME_LENS[k % len(RENAME_LENS)]
    while True:
        cand = ''.join(random.choice(ALPHA) for _ in range(ln))
        full = cand + ('.' + ext if ext else '')
        if full.upper() not in busy:
            break
    busy.add(full.upper())
    r[0] = full
    k += 1
...
need = sum(max(0, -(-r[2] // csz)) for r in short_files)
if need > nclusters:
...
for n, p, sz, mt, ro, orig in short_files:
    n11 = pack83(n)
    taken.add(n11)
    cl = put(sz, p)
    root[i*32:(i+1)*32] = dirent(n11, 0x01 if ro else 0x20, cl, sz, mt)
    log_short.append((i, orig, n, n11.decode(), cl, sz))
    i += 1
...
for i, orig, n, n11, cl, sz in log_short:
    mark = "  ПЕРЕЙМЕНОВАНО з %s" % orig if n != orig else ""
    print("            запис %-3d  %-8s %-3s кластер %-4d %6d Б   <- %s%s"
          % (i, n11[:8].strip(), n11[8:].strip(), cl, sz, n, mark))
print("            записи %d..%d — нулі (0x00 = кінець каталогу)" % (used_end, ROOTENT - 1))
print()
if long_files:
    print("ПРОПУЩЕНО (імена не 8.3, на флешку не пішли):")
    for r in long_files:
        print("            %-40s %d Б" % (r[0], r[2]))
    print()
print("записів LFN у томі: 0. Затички проти вінди немає.")
print("У корені лише мітка тому і %d файлів 8.3." % len(log_short))
ren = [(o, n) for _, o, n, _, _, _ in log_short if o != n]
if ren:
    print()
    print("ТАБЛИЦЯ ВІДПОВІДНОСТІ (запиши, щоб звірити з тим, що покаже стійка):")
    for o, n in ren:
        print("            %-14s -> %-14s (основа %d символів)"
              % (o, n, len(n.partition('.')[0])))
PYEOF

...
# Правило розміщення одне: імена, що влазять у 8.3, ідуть на початок кореня
# і на перші кластери; імена, що не влазять (довгі, з пробілами, з малими
# літерами), ідуть у САМИЙ ХВІСТ кореня разом зі своїми записами LFN.
# Тобто "System Volume Information" з f0 лягає останнім — це той самий
# нульовий файл з f0, а не вигаданий каталог.
#
...
IMG="$IMGDIR/720k-lba63-f0_$(date +%Y%m%d-%H%M%S).img"
...
for n, p, sz, mt, ro in short_files:
    ...
    log_head.append((i, n, n11.decode(), cl, sz))
    i += 1
head_end = i

# довгі імена — у самий хвіст кореня, останній файл упирається в останній запис
tail = ROOTENT
for n, p, sz, mt, ro in reversed(long_files):
    n11 = alias(n, taken)
    taken.add(n11)
    cl = put(sz, p)
    ...
    for k, e in enumerate(ents):
        root[(start+k)*32:(start+k+1)*32] = e
    log_tail.insert(0, (start, tail - 1, n, n11.decode(), cl, sz, len(ents) - 1))
    tail = start
...
print("корінь:     запис 0    мітка AGIE")
for i, n, n11, cl, sz in log_head:
    print("            запис %-3d  %-11s кластер %-4d %6d Б   <- %s" % (i, n11, cl, sz, n))
if log_tail:
    print("            записи %d..%d — нулі" % (head_end, log_tail[0][0] - 1))
for a, b_, n, n11, cl, sz, nl in log_tail:
    print("            записи %d..%d  %d LFN + %-11s кластер %-4d %6d Б   <- %s"
          % (a, b_, nl, n11, cl, sz, n))
    if cl == 0:
        print("                       розмір 0 -> кластер не виділявся, в даних нічого немає")
print("            останній запис кореня — %d з %d" % (ROOTENT - 1, ROOTENT))
PYEOF

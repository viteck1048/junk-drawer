...
//
// ТІЛЬКИ ASCII, і на екрані, і у вводі, і в коментарі всередині образу.
// Це не сором'язливість, а висновок із цеху: кодову сторінку консолі там
// кожна машина піднімає свою — єдина XP і майже всі Win7 стають у 866,
// одна Win7 у 437, Win10 у 1251. Що не постав примусово, десь та й вийде
// сміття, ще й у чужих руках і без нас. Знаки 0x20..0x7E однакові в усіх
// трьох, тому єдине, що працює скрізь однаково, — латинка. Кодових
// сторінок утиліти не чіпають зовсім.
//
...
static void print_name_rules(void)
{
    say("\r\n   Name rules inside the image:\r\n"
        ...
        "   Pravila za imenata vatre v obraza:\r\n"
        "     do 8 znaka, tochka, do 3 znaka;\r\n"
        "     samo glavni bukvi A-Z, tsifri 0-9 i - _ ;\r\n"
        "     malkite bukvi se priemat i stavat glavni;\r\n"
        "     bez interval, bez kirilitsa, bez dalgi imena.\r\n");
}
...
/* ABOUT_ME.TXT: перший рядок — коментар до образу, за ним CRLF, далі
   ...
   Знаки — тільки друкований ASCII. Старі образи (рівно 50 знаків,
   доповнені пробілами, і той самий CRLF) читаються тим самим кодом:
   хвостові пробіли обрізаються й від них нічого не лишається.
...
/* Коментар образу в out[]. 0 = коментаря немає.
...
static int read_about(const char *path, char *out, size_t cap)
{
    ...
    /* До CRLF (та й до будь-якого керівного знака), далі — не наша справа.
       Байт поза ASCII — це коментар, писаний не нами (скажімо, кирилицею в
       чиємусь кодуванні). Показуємо його знаком питання: інакше кожна
       машина малювала б своє, і незрозуміло було б навіть, що це чуже. */
    for (n = 0; n < (int)want && n < NOTE_MAX; n++) {
        if (data[n] < 32 || data[n] == 127)
            break;
        out[n] = (data[n] > 126) ? '?' : (char)data[n];
    }
    while (n > 0 && out[n - 1] == ' ')
        n--;
    out[n] = 0;
